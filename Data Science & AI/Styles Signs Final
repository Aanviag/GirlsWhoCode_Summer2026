...
X_train, X_validate, y_train, y_validate = train_test_split(X_train, y_train, test_size=20, random_state=42)

neural_net_model = MLPClassifier( hidden_layer_sizes=(64),random_state=42,tol=0.005)
...
# Confusion matrix & misidentidfied letters
mistake_counts = defaultdict(int)

for true, pred in zip(y_test, y_pred):
    if true != pred:
        mistake_counts[(true, pred)] += 1

# Sort mistakes by frequency
sorted_mistakes = sorted(mistake_counts.items(), key=lambda x: x[1], reverse=True)

print("\nTop 3 most misidentified letters:")
for (true, pred), count in sorted_mistakes[:3]:
    print(f"Letter {true} is most often mistaken for {pred} ({count} times)")
